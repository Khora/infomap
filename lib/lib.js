...
function searchTable(isMobile, filterOnFavorites) {
        ...
        for (j = i; j < i + lengthOfDataset && j < tr.length; j++) {
            ...
            for (k = 0; k < td.length; k++) {
                if (matchingMethod(td[k].innerHTML, filter)) {
                    containsText = true;
                    currentlyVisibleElements.push(Math.ceil(i / lengthOfDataset));
                }
            }
        }
    ...
    var currentlyVisibleElementsUnique = currentlyVisibleElements.filter((v, i, a) => a.indexOf(v) === i); 
    
    favoritesList = getFavorites();
    if (map !== null && map !== undefined) {
        removeAllMarkerFromLeafletMap();
        for (i = 0; i < currentlyVisibleElementsUnique.length; i++) {
            if (!filterOnFavorites || favoritesList.includes(currentlyVisibleElementsUnique[i])) {
                //showInMap(currentlyVisibleElementsUnique[i]);
            }
        }
    }
}
...
function updateStarImages(idsToUpdate) {
    ...
    for (i = 0; i < idsToUpdate.length; i++) {
        if (currentFavoritesIds.indexOf(idsToUpdate[i]) >= 0) {
            if (document.getElementById(starElementPrefix + idsToUpdate[i]) != undefined) {
...
function doTaskAfterTimeWhenNotRescheduled(functionToCall, delayMs) {
    doNotExecuteTaskBeforeMap.set(functionToCall, getCurrentUnixtimeMillis() + delayMs);
    setTimeout(function() { checkScheduledTask(functionToCall); }, delayMs + 10);
}

function checkScheduledTask(functionToCall) {
    if (doNotExecuteTaskBeforeMap.get(functionToCall) <= getCurrentUnixtimeMillis()) {
        doNotExecuteTaskBeforeMap.delete(functionToCall);
        try {
            eval(functionToCall);
        } catch (e) {
            console.log("Could not call Function: " + functionToCall);
        }
    }
}

function getCurrentUnixtimeMillis() {
    var d = new Date();
    return d.getTime();
}