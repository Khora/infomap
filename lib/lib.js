...
function toggleAllFavoritesAndUpdateStarImages() {
    favoritesToggled = removeElementsFromArray(starImageElements, getFavorites());
    rewriteCookiesWithFavorites(favoritesToggled);
    updateStarImages(starImageElements);
}
...
function toggleFavorites(listOfIds) {
    ...
    for (i = 0; i < listOfIds.length; i++) { 
        if (currentFavoritesIds.indexOf(listOfIds[i]) < 0) {
            addToFavorites([listOfIds[i]]);
        } else {
            removeFromFavorites([listOfIds[i]]);
        }
    }
}
